...
#Данный файл содержит в себе правила унификации символов юникода для сокращения рабочего числа букв в дальнейшей обработке текста
#Данный модуль является обязательным в общей цепочке обработки, и сейчас заточен под русский язык.
#Чтобы адаптировать его для других языков, достаточно адаптировать список alphabet под стандартную клавиатуру на этом языке, а также проверить, что спецсимволы с точками не входят в состав алфавита этого языка
...
def open_lines(text):
    #text = io.open(filename,'r', encoding='utf-8').readlines()
    lines = []
    for line in text:
...
# фнукция использует дефолтную библиотеку
...
def unify_sym(text): #принимает строку в юникоде
    ...
    #для дефиса и тире - (минус) предлагаю не различать дефис и тире, как знаки препинания, кроме случаев, где дефис разделяет части слова.
    #для пробела - неразрывный заменяется на обычный, несколько пробелов подряд заменяется на 1,l то же самое для табуляций
    ...
    #короткое тире, среднее тире, длинное тире, цифровое тире, неразрывный дефис, дефис-минус, дефис, макроны и так далее 
    ...
    #сами по себе умляуты, акуты, грависы мы игнорируем, оставляем только буквы с точками - и их переводим в обычные латинские
    #для работы с малыми языками России, такими как удмурсткий, хантыйский, саамский и пр. следует добавить к этому списку расширенную кириллицу
